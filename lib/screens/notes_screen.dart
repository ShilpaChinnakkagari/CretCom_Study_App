...
class _NotesScreenState extends State<NotesScreen> with TickerProviderStateMixin {
  ...
  // ✅ ADDED: Get cached PDF file
  Future<File?> _getCachedPdf(String fileName, String fileId) async {
    try {
      final dir = await getApplicationDocumentsDirectory();
      final unitCacheDir = Directory('${dir.path}/${widget.unit.id}');
      
      if (!await unitCacheDir.exists()) {
        await unitCacheDir.create();
      }
      
      final cachedFile = File('${unitCacheDir.path}/$fileName');
      ...
      await _cleanOldCache(unitCacheDir);
      ...
      if (response is drive.Media) {
        ...
        print("✅ Cached: $fileName (${(fileSize/1024/1024).toStringAsFixed(2)}MB)");
  ...
  // ✅ ADDED: Clean old cache files
  Future<void> _cleanOldCache(Directory cacheDir) async {
    try {
      if (!await cacheDir.exists()) return;
      
      final files = await cacheDir.list().toList();
      if (files.length < _maxFilesPerUnit) return;
      ...
      int toDelete = files.length - _maxFilesPerUnit;
      for (int i = 0; i < toDelete; i++) {
        await files[i].delete();
        print("🗑️ Deleted old cache: ${files[i].path}");
      }
    } catch (e) {
  ...
  // ✅ ADDED: Clear unit cache
  Future<void> _clearUnitCache() async {
    try {
      final dir = await getApplicationDocumentsDirectory();
      final unitCacheDir = Directory('${dir.path}/${widget.unit.id}');
      ...
      if (await unitCacheDir.exists()) {
        int count = 0;
        await for (var file in unitCacheDir.list()) {
          await file.delete();
          count++;
        }
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Cleared $count cached files')),
          );
        }
      }
    } catch (e) {
  ...
  // ✅ CHANGED: Now uses cached PDF loading
  Future<void> _viewNote(Note note) async {
    try {
      ...
      // ✅ Use cached PDF instead of downloading every time
      final file = await _getCachedPdf(note.title, note.fileId!);
